...
def evaluate_logits_alltasks(model, loader, splits, num_classes):
    ...
    all_labels = []
    all_preds = []
    per_task_labels = [[] for _ in splits]
    per_task_preds = [[] for _ in splits]

    with torch.no_grad(), autocast():
        for inputs, labels in tqdm(loader, desc='Evaluating multihead model'):
            inputs, labels = inputs.to(device), labels.to(device)
            ...
            if isinstance(outputs, list):
                for i in range(len(outputs)):
                    exclude_labels = torch.tensor(
                        [l for l in all_splits.cpu().numpy() if l not in splits[i].cpu().numpy()],
            ...
            for i, label in enumerate(labels.cpu().numpy()):
                per_task_labels[task_idx[i]].append(label)
                per_task_preds[task_idx[i]].append(outputs.cpu().numpy()[i])

            correct += (labels == outputs2).sum().item()
            total += labels.size(0)

    # Joint F1 Score
    joint_f1 = f1_score(all_labels, all_preds, average='macro')

    # Per Task F1 Scores
    task_f1_scores = [f1_score(per_task_labels[i], per_task_preds[i], average='macro') if per_task_labels[i] else 0 for i in range(len(splits))]
    avg_per_task_f1 = sum(task_f1_scores) / len(task_f1_scores)

    # Calculate Split Accuracies
    split_accs = [0] * len(splits)
    for i, split in enumerate(splits):
        split_labels = [lbl for lbl in all_labels if lbl in split.cpu().numpy()]
        split_preds = [prd for lbl, prd in zip(all_labels, all_preds) if lbl in split.cpu().numpy()]
        split_accs[i] = f1_score(split_labels, split_preds, average='macro')

    return correct / total, sum(split_accs) / len(split_accs), split_accs, joint_f1, avg_per_task_f1, task_f1_scores

# Example usage
# model = YourModel()
# loader = YourDataLoader()
# splits = YourSplits()  # e.g., [[0, 1, 2], [3, 4, 5]]
# num_classes = YourNumClasses()
# evaluate_logits_alltasks(model, loader, splits, num_classes)
...
def train_logits(model, train_loader, test_loader, learning_rate, epochs=200,  remap_class_idxs=None):
...
def evaluate_model(eval_type, model, config, splits, **opt_kwargs):
    ...
    if eval_type == 'logits':    
        acc_overall, acc_avg, perclass_acc, joint_f1, avg_per_task_f1, task_f1_scores = evaluate_logits_alltasks(
            model, loader, 
            splits=[splits,splits],#config['dataset']['class_splits'], 
    ...
    results['Joint F1']= joint_f1
    results['Per Task F1'] = avg_per_task_f1
    results['task F1 scores'] = task_f1_scores
    # torch.save({f'{config['dataset']_config['']}'}, model)
...
def prepare_data(config, device='cuda'):
    ...
    if data_config['type'] == 'cifar':
        ...
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
    elif data_config['type'] == 'isic2019':
        from datasets.isic2019 import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
    
    elif data_config['type'] == 'rsna':
        from datasets.rsna import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)

    elif data_config['type'] == 'aptos':
        from datasets.aptos import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
...
def prepare_densenet(config, device):
    ...
    if 'densenet121' in name:
        from torchvision.models import densenet121 as wrapper
    else:
    ...
    for base_path in tqdm(config['bases'], desc="Preparing Models"):
        ...
        for key in base_sd.keys():
            if key.startswith('classifier.0'):
                new_key = key.replace('classifier.0', 'classifier', 1)
...
def prepare_models(config, device='cuda'):
    ...
    elif config['name'].startswith('densenet'):
        return prepare_densenet(config, device)
...
def prepare_graph(config):
    ...
    if config['name'].startswith('resnet'):
        ...
        import graphs.densenet_graph as graph_module
        graph = getattr(graph_module, model_name)
    else:
        raise NotImplementedError(config['name'])
    return graph
...
def inject_pair(config, pair, ignore_bases=False):
    ...
    if not ignore_bases:
        config['model']['bases'] = pair#[os.path.join(config['model']['dir'], split, f'{model_name}_v0.pth.tar') for split in pair]
    return config
