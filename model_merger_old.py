...
#                     print(f"Stopped At {self.stop_at}.")
#                     break
...
class MergeHandler:
    ...
    def handle_batchnorm2d(self, forward, node, module):
        ...
        if forward:
            ...
            for parameter_name in ['weight', 'bias', 'running_mean', 'running_var']:
                ...
                # merge = torch.ones(parameter.shape+parameter.shape).cuda()
                parameter.data = merge @ parameter
    ...
    def handle_conv2d(self, forward, node, module):
        ...
        if forward: # unmerge
            print("-----------")
            print("Handle Conv2D - Forward")
            print(f"self.unmerge.shape={self.unmerge.shape}")
            print("-----------")
            try:
                module.weight.data = torch.einsum('OIHW,IU->OUHW', module.weight, self.unmerge)
            except:
            ...
            print("-----------")
            print("Handle Conv2D - Backward")
            print(f"self.merge.shape={self.merge.shape}")
            print("-----------")
            try:
                module.weight.data = torch.einsum('UO,OIHW->UIHW', self.merge, module.weight)
            except:
    ...
    def prop_back(self, node):
        ...
        if info['type'] in (NodeType.OUTPUT, NodeType.INPUT):
            ...
            # merge = self.merge.chunk(2, dim=1)
            # merge = self.merge.clone()
            # chunks = merge.chunk(merge.shape[0]//parameter.shape[0], dim=1)[0]
            pred_modules_names = [self.graph.get_node_info(x)['layer'] for x in self.graph.preds(node)]
            pred_modules = [self.graph.get_module(x) for x in pred_modules_names]
            pred_modules_params = [getattr(m, 'weight').shape[0] for m in pred_modules]
            start = 0
            # end = 0
            # merge = []
            # for p in pred_modules_params:
            #     end += p
            #     merge.append(self.merge[:,start: end])
                # start += p
            ...
            for pred, m in zip(self.graph.preds(node), merge):
                ...
                print("Backward")
                print(f"node={node}")
                print(f"merge.shape = {m.shape}")
                print("---------------")
            # for pred in self.graph.preds(node):
                # dim = self.graph.get_module(self.graph.get_node_info(pred)['layer']).weight.shape[0]
                # m = self.merge[:,:dim]
                MergeHandler(self.graph, m, self.unmerge).prop_back(pred)
                # start += dim
        elif info['type'] == NodeType.MODULE:
    ...
    def prop_forward(self, node):
        ...
        if info['type'] in (NodeType.OUTPUT, NodeType.INPUT):
            ...
            print(f"self.merge.shape={self.merge.shape}")
            print(f"num_to_concat={num_to_concat}")
            print("--------------")
